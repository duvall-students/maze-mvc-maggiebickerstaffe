...
public class MazeDisplay extends Application {
	...
	private Group setupMaze(){
		...
		mirrorMaze = new Rectangle[(mazeController.getRows())][(mazeController.getColumns())];
		for(int i = 0; i< (mazeController.getRows()); i++){
			for(int j =0; j < (mazeController.getColumns()); j++){
	...
	private Scene setupScene () {
		...
		Scene scene = new Scene(root, (mazeController.getColumns())*BLOCK_SIZE+ EXTRA_HORIZONTAL, 
				(mazeController.getRows())*BLOCK_SIZE + EXTRA_VERTICAL, Color.ANTIQUEWHITE);
	...
	private HBox setupControlButtons(){
		...
		stepButton.setOnAction(value ->  {
			this.mazeController.doOneStep(MILLISECOND_DELAY);
		});
